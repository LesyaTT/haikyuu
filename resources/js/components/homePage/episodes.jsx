...
export const Episodes = () => {
   ...
   const info = importData('episodes');

  return (
    <>
      <section id="episodes">
        <div className="flex justify-between items-center">
          <SecTitle title={"New Episodes"} />
          <OrangeButton info={"See all episodes"} />
        </div>
        <div className="flex justify-between mt-10">
            {info.map(item => (
                <EpisodCard
                    title={`Season ${item.season}, series ${item.series}`}
                    date={item.date}
                    views={`${item.views}k`}
                    img={`${item.img_path}`}
                />
...
const EpisodCard = ({ title, date, views, img }) => {
  return (
    <>
      <div className="">
        <img
